...
solve (n, m, grid') = show mans ++ " " ++ show (length $ filter (==mans) answer) ++ "\n"
  where
    ...
    answer = parMap (walking grid) [idx | idx <- range bnds, let ch = grid ! idx, ch /= '.']
